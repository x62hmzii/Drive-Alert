...
import com.example.driverdrowsinessdetectionsystem.camera.CameraSourcePreview;
import com.example.driverdrowsinessdetectionsystem.camera.GraphicOverlay;
...
public class FaceTrackerActivity extends AppCompatActivity {
    ...
    private String key_2 = "Final Year project";
...
// 1. Handle start time (modified)
        startTime = intent_2.getStringExtra("start_time_key");  // Using consistent key
        if (startTime == null || startTime.isEmpty()) {
            // Generate current time if not provided
            startTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(new Date());
        }

// 2. Handle sensitivity (unchanged, keep your existing logic)
        String time_info = intent_2.getStringExtra(key_4);  // key_4 = "sensitivity"
        if (time_info != null) {
        ...
        end_button.setOnClickListener(view -> {
            ...
            count = 0;
            count1 = 0;
            next.putExtra(Constants.KEY_START_TIME, startTime);
            next.putExtra(Constants.KEY_DETECTION_RESULT, tv_1.getText().toString());
            next.putExtra(Constants.KEY_SENSITIVITY, String.valueOf(s_status));
            next.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
            startActivity(next);
            finish();
        });

        end_button.setOnTouchListener((v, event) -> {
            Intent next = new Intent(FaceTrackerActivity.this, EndActivity.class);
            count = 0;
            count1 = 0;
            next.putExtra(Constants.KEY_START_TIME, startTime);
    ...
    private class GraphicFaceTracker extends Tracker<Face> {
        ...
        private com.example.driverdrowsinessdetectionsystem.FaceGraphic mFaceGraphic;

        GraphicFaceTracker(GraphicOverlay overlay) {
            mOverlay = overlay;
            mFaceGraphic = new com.example.driverdrowsinessdetectionsystem.FaceGraphic(overlay);
        }
